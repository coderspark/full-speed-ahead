...
func _ready() -> void:
	...
	if Global.SaveFileLoaded:
		RAND.seed = Global.SaveFile.CurrentLevelData["Seed"]
		SEED = RAND.seed
	else:
		RAND.randomize()
		SEED = RAND.seed
	...
	var map = []
	for n in range(Global.LevelData[Global.LevelName].LengthTiles):
		...
		var tplate = 0
		while true:
			tplate = RAND.randi_range(0, number_of_templates-1)
	...
	paste_template(-3, 18+(10*len(map)))
